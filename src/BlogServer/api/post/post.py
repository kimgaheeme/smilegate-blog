...
async def create_post(userId: int, userRequest: CreatePostRequest = Depends(), postImg: Optional[UploadFile] = File(...)):

    if postImg is not File(...):
...
async def update_post(postid: int, postImg: Optional[UploadFile] = File(...),
